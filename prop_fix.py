...
for i in range(num_props):
    mesh = sim['system_model.wig.wig.wig.operation.input_model.p'+str(i)+'b1_mesh_rotor.p'+str(i)+'b1_mesh']

    ax.plot_trisurf(mesh[:,:,0].flatten(), mesh[:,:,1].flatten(), mesh[:,:,2].flatten())

    #for j in range(num_blades):
    for j in range(2):
        fmesh = sim['system_model.wig.wig.wig.operation.input_model.p'+str(i)+'b1_mesh_rotor.p'+str(i)+'b1_mesh_rotor'+str(j)]/0.3048
        print(np.shape(fmesh))
        ax.plot_trisurf(fmesh[0,:,:,0].flatten(), fmesh[0,:,:,1].flatten(), fmesh[0,:,:,2].flatten())
