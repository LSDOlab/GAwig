...
for i in range(num_surfaces):
    ...
    mesh_val = np.zeros((num_nodes, nx[i], ny[i], 3))
    ...
    uvlm_parameters.append(('wing' + str(i) + '_coll_vel', True, np.zeros((num_nodes,nx[i]-1,ny[i]-1,3))))
    surface_names.append('wing' + str(i))
    surface_shapes.append((nx[i],ny[i],3))
...
for i in range(len(surface_names)):
    ...
    # nx = surface_shape[0]
    # ny = surface_shape[1]
    initial_conditions.append((gamma_w_0_name, np.zeros((num_nodes-1, ny[i] - 1))))
    initial_conditions.append((wake_coords_0_name, np.zeros((num_nodes-1, ny[i], 3))))
...
panel_forces, eval_pts, int1, _, _, _, = uvlm_op.evaluate()
