...
for s_ind in range(num_surfaces):
    ...
    panels = mesh.copy()[:,:int((ny+1)/2),:]
    panels[:,:,1] += s_ind*y_delta

    total_mesh_data[s_ind, :,:,:] = panels
    
    surface_names = []
    surface_shapes = []
    mesh_vals = []

    for dummy_ind in range(s_ind+1):
        expanded_panel = np.zeros((num_nodes, nx, int((ny+1)/2), 3))
        for j in range(num_nodes):
            expanded_panel[j, :, :, 0] = total_mesh_data.copy()[dummy_ind, :, :, 0]
            expanded_panel[j, :, :, 1] = total_mesh_data.copy()[dummy_ind, :, :, 1]
            expanded_panel[j, :, :, 2] = total_mesh_data.copy()[dummy_ind, :, :, 2]

        uvlm_parameters.append((f'panel_{dummy_ind}', True, expanded_panel))
        surface_names.append(f'panel_{dummy_ind}')
        surface_shapes.append((nx, int((ny+1)/2), 3))

        mesh_vals.append(expanded_panel)
    ...
    for j, surf_name in enumerate(surface_names):
        lift_array[s_ind, j, :] = sim[f'operation.post_processor.ThrustDrag.{surf_name}_L_panel'][-1].reshape(int(nx-1), int((ny-1)/2))[chord_station,:]
        gamma_b_array[s_ind, j, :] = sim[f'operation.post_processor.ThrustDrag.{surf_name}_gamma_b'][-1].reshape(int(nx-1), int((ny-1)/2))[chord_station,:]
        circulations_array[s_ind, j, :] = sim[f'operation.post_processor.ThrustDrag.{surf_name}_circulations'][-1].reshape(int(nx-1), int((ny-1)/2))[chord_station,:]
        y_positions[s_ind, j, :] = (mesh_vals[j][0][1,:-1,1] + mesh_vals[j][0][1,1:,1])/2
...
color = plt.cm.rainbow(np.linspace(0, 1, num_surfaces))
if mode == 'lift':
    data = lift_array
...
for i in range(num_surfaces):
    for j in range(i+1):
        if j == 0:
            if i == 0:
                plt.plot(y_positions[i, j, :], data[i, j, :], linewidth=2, c=color[i], label=f'{str(i+1)} surface')
            else:
                plt.plot(y_positions[i, j, :], data[i, j, :], linewidth=2, c=color[i], label=f'{str(i+1)} surfaces')
        else:
            plt.plot(y_positions[i, j, :], data[i, j, :], linewidth=2, c=color[i])
