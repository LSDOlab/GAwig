    ...
    def define(self):
        ...
        for i in range(num_blades):
            ...
            for j in range(nt):
                ...
                # test compute the angles for all j values:
                # angle = self.create_input('angles'+str(j), val=(set_angle + rad_per_dt_np * np.arange(nt) * dir)[:, np.newaxis].flatten())
                rot_mat = self.create_output('rot_mat_' + str(i), shape=(3,3), val=0)
