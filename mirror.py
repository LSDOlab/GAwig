...
class MirrorCSDL(ModuleCSDL):
    ...
    def define(self):
        ...
        mesh_in = self.declare_variable(mesh_name, shape=(nt,nc,ns,3))
...
# a separate propeller mirror model because I don't want to deal with different shapes...
class PropMirror(m3l.ExplicitOperation):
    def initialize(self, kwargs):
        self.parameters.declare('component', default=None)
        self.parameters.declare('mesh', default=None)
        self.num_nodes = None
    ...
    def compute(self):
        ...
        nt = self.parameters['nt']
        point = self.parameters['point']
        csdl_model = PropMirrorCSDL(module=self,mesh_name=mesh_name,num_blades=num_blades,ns=ns,nc=nc,nt=nt,point=point)
        return csdl_model
    ...
    def evaluate(self):
        ...
        mirror_mesh = m3l.Variable(mesh_name + '_mirror', shape=(num_blades,nt,nc,ns,3), operation=self)
        mesh_out = m3l.Variable(mesh_name + '_out', shape=(num_blades,nt,nc,ns,3), operation=self)
...
class PropMirrorCSDL(ModuleCSDL):
    def initialize(self):
        self.parameters.declare('mesh_name')
        self.parameters.declare('num_blades')
        self.parameters.declare('ns')
    ...
    def define(self):
        ...
        mesh_in = self.declare_variable(mesh_name, shape=(num_blades,nt,nc,ns,3))
        #mesh_in = self.register_module_input(mesh_name, shape=(nt,nc,ns,3), promotes=True)
        self.register_output('debug_mesh', 1*mesh_in)

        # rotate the mesh:
        translated_mesh_points = mesh_in - np.tile(point, (num_blades, nt, nc, ns, 1))
        rotated_mesh_points = self.create_output('rotated_mesh_points', shape=(num_blades,nt,nc,ns,3), val=0)
        for l in range(num_blades):
            for i in range(nt):
                for j in range(nc):
                    for k in range(ns):
                        eval_point = csdl.reshape(translated_mesh_points[l,i,j,k,:], (3))
                        #eval_point = csdl.reshape(mesh_in[i,j,:], (3))
                        rotated_mesh_points[l,i,j,k,:] = csdl.reshape(csdl.matvec(rotation_matrix_y, eval_point), (1,1,1,1,3))

        rotated_mesh = rotated_mesh_points + np.tile(point, (num_blades, nt, nc, ns, 1))

        # translate the mesh based on altitude:
        dh = self.create_output('dh', shape=(num_blades,nt,nc,ns,3), val=0)
        dh[:,:,:,:,2] = csdl.expand(h, (num_blades,nt,nc,ns,1), 'i->abcdi')
        ...
        # create the mirrored mesh:
        mirror = self.create_output(mesh_name+'_mirror', shape=(num_blades,nt,nc,ns,3), val=0)
        mirror[:,:,:,:,0] = mesh[:,:,:,:,0]
        mirror[:,:,:,:,1] = mesh[:,:,:,:,1]
        mirror[:,:,:,:,2] = -1*mesh[:,:,:,:,2]
